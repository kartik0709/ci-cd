...
// Update an account's name, description, or active status (code and type are immutable once created)
query "accounts/{account_id}" verb=PATCH {
  ...
  input {
    ...
    // Pass the string "true" or "false". A native JSON boolean is not usable here: this platform's XanoScript engine cannot distinguish an explicit false from an omitted field for boolean inputs (confirmed empirically), so the value is accepted as text instead.
  ...
  guid = "SVBi-57JDp6tYnFJD2VMbqIDIhE"
}