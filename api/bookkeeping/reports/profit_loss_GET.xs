...
query "reports/profit-loss" verb=GET {
  ...
  stack {
    ...
    foreach ($income_accounts) {
      each as $acct {
        ...
        array.push $income_lines {
          value = {account: $acct, balance: $bal.balance}
        }
      
        math.add $total_income {
          value = $bal.balance
        }
      }
    }
    ...
    foreach ($expense_accounts) {
      each as $acct {
        ...
        array.push $expense_lines {
          value = {account: $acct, balance: $bal.balance}
        }
      
        math.add $total_expense {
          value = $bal.balance
        }
      }
    }
  ...
  guid = "sjeBTdjfzlfMDyym8Iv8_s5vnFY"
}