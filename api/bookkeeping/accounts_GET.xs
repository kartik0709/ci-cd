...
// List chart-of-accounts entries, optionally including each account's current balance
query accounts verb=GET {
      ...
      if ($input.with_balance) {
        ...
        foreach ($accounts_page.items) {
          each as $acct {
            ...
            array.push $enriched {
              value = $acct|set:"balance":$bal.balance
            }
          }
        }
      
        var.update $accounts_page {
          value = $accounts_page|set:"items":$enriched
        }
      }
    }
  }
  ...
  guid = "zIcEiNAVkcOHFfXOxAOOlTOnUdI"
}